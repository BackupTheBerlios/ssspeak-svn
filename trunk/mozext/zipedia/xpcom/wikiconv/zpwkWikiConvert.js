...
var WikiConvertModule = {
  myCID : Components.ID("{3212358c-6f34-4c23-a0cf-d3d3da9e4e5a}"),
  myProgID : "@nargila.org/wiki-convert;1",
  
  registerSelf : function(compMgr, fileSpec, location, type) {
    compMgr = compMgr.QueryInterface(Components.interfaces.nsIComponentRegistrar);
    compMgr.registerFactoryLocation(
				    this.myCID,
				    "Wiki Format to HTML Converter",
				    this.myProgID,
				    fileSpec,
				    location,
				    type
  ...
  myFactory : {
	...
	throw Components.results.NS_ERROR_NO_AGGREGATION;
      ...
      return new WikiConvert();
    }
  }
}

function NSGetModule(compMgr, fileSpec) {
    return WikiConvertModule;
}
...
WikiConvert.prototype = {
  _location: null,
  QueryInterface : function(iid) {
    if (!iid.equals(Components.interfaces.zpwkIWikiConvert) &&
	!iid.equals(Components.interfaces.nsISupports)) {
      throw Components.results.NS_ERROR_NO_INTERFACE;
    }
    return this;
  },
  convert: function(wiki, location) {

    var txt = this.preprocessPage(wiki);
	  
    txt = this.addToc(txt);
	  
    html = InstaView.convert(txt, location);	

    return html;
  },
  preprocessPage: function(txt) {
    return this.handleTemplates(txt);
  },
  findTemplateEnd: function(txt, pos) {
    var count = 1;
    var i2 = pos;

    for (i2 = pos; count && i2 < txt.length; ++i2) {
      if (txt.charAt(i2) == '{' && txt.charAt(i2 + 1) == '{') {
	count++;
	++i2;
	continue;
      }
  ...
  handleTemplates: function(txt) {
    var res = "";

    for (var i1 = txt.indexOf("{{"); i1 >= 0; i1 = txt.indexOf("{{")) {
      var i2 = this.findTemplateEnd(txt, i1 + 2);
  ...
  addToc: function(txt) {
    ...
    for (var i = 0; i < lines.length; ++i) {
      var r = lines[i].match(/^(={1,6})\W*(\w+(\W+\w+)*)\W*\1(.*)$/);

      if (r) {
	count++;

	for (var j = 0; j < r[1].length; ++j) {	// write as many '#' as there are '='
	  toc += "#";
	}
	toc += " [[#" + r[2].replace(/ /g, '_') + "|" + r[2] + "]]\n";
      }
    }
    ...
    if (count > 1) {
      ...
      res = txt.substr(0, pos);
      res += "\n";
      res += "<span class='toc' id='toc'>\n<strong>Contents:</strong><br/>\n" + toc + "\n</span>\n";
      res += "\n";
      res += txt.substr(pos);
    } else {
      res = txt;
...
InstaView.dump = function(from, to)
{
  ...
  InstaView.convert = function(wiki, location)
{
	...
	// begin parsing
		for (;remain();) if ($(/^(={1,6})\W*(\w+(\W+\w+)*)\W*\1(.*)$/))/* ($(/^(={1,6})(.*)\1(.*)$/)) */ {
	  p=0;
	  var name = parse_inline_nowiki($r[2]);
	  var target = name.replace(/ /g, '_');
	  endl(f('<a name="?" id="?"/><h?>?</h?>\n?', target, target, $r[1].length, name, $r[1].length, $r[4]));
